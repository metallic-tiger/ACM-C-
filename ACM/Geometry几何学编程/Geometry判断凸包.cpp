...
*/
bool mult(Point Base,Point A,Point B){
    return (A.x-Base.x)*(B.y-Base.y)>=(B.x-Base.x)*(A.y-Base.y);
}
...
int Min=0;
    for(int i=0;i<All.lenth;i++){
        if(All.P[i].x>All.P[Max].x)
            Max=i;
        }
WALL.P[0]=All.P[Max];
WALL.lenth+=1;

    for(int i=Max;i<All.lenth-1;i++){
        All.P[i]=All.P[i+1];
        }
All.lenth--;
...
*/
    for(int i=0;i<All.lenth/2;i++){
        for(int j=i;j<All.lenth-i;j++)
            {
            if(mult(WALL.P[0],All.P[i],All.P[Max]))
                Max=i;
            if(!mult(WALL.P[0],All.P[i],All.P[Min]))
                Min=i;
            }
    All.P[i]=All.P[Max];
...
的方法
*/
    for(int i=0;i<All.lenth-1;i++){
        if(mult(WALL.P[WALL.lenth-1],All.P[i],All.P[i+1]))
            {
